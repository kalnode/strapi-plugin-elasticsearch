/*
 *
 * Configure collection list page
 *
 */
...
import { Alert, Loader, Box, Flex, Button, Table, Thead, Tbody, Tr, Td, Th, Textarea, TwoColsLayout, Divider, Typography, IconButton, ModalLayout, ModalBody, ModalHeader, ModalFooter } from '@strapi/design-system'
import { Pencil, Server } from '@strapi/icons'

import { apiGetContentConfig, apiRequestCollectionIndexing, apiImportContentConfig, apiExportContentConfig } from "../../utils/apiUrls"
import axiosInstance  from '../../utils/axiosInstance'

const exportContentConfig = () => {
    return axiosInstance.get(apiGetContentConfig, { responseType: 'blob' })
    .then((response) => {
        const href = URL.createObjectURL(response.data)
        const link = document.createElement('a')
        link.href = href
        link.setAttribute('download', 'strapi-plugin-esplugin-contentconfig.json')
...
const importContentConfig = (conf) => {
    return axiosInstance.post(apiImportContentConfig, { data: conf })
}
...
const PageConfigure = () => {
    ...
    const performImport = () => {
        ...
        if (conf && conf.length > 0) {
            ...
            .then(() => {
                toggleNotification({
                    type: "success", message: "Collections configuration imported. Please refresh this view.",
                    timeout: 5000
                })
            })
            .catch((err) => {
                toggleNotification({
                    type: "warning", message: "Importing collections configuration failed. An error was encountered.",
                    timeout: 5000
                })
                console.log(err)
            })
    ...
    const performExport = () => {
        ...
        .then(() => {
            toggleNotification({
                type: "success", message: "Collections configuration exported.",
                timeout: 5000
            })
        })
        .catch((err) => {
            toggleNotification({
                type: "warning", message: "Exporting collections configuration failed. An error was encountered.",
                timeout: 5000
            })
            console.log(err)
        })
    ...
    useEffect(() => {
        ...
        .catch((err) => {
            toggleNotification({
                type: "warning", message: "An error was encountered while fetching the configuration.",
                timeout: 5000
            })
            console.log(err)
        })
                                ...
                                <Table colCount={4} rowCount={config.length}>
                                    ...
                                    <Tbody>
                                        ...
                                        config.map((collection, idx) => {
                                            ...
                                            <Tr key={idx}>
                                                <Td>
                                                    <Typography textColor="neutral600">{collection.collectionName}</Typography>
                                                </Td>
                                                <Td>
