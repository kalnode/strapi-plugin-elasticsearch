...
import { Link, useTracking, LoadingIndicatorPage, useNotification } from '@strapi/helper-plugin'
import pluginId from '../../pluginId'
import { apiGetCollectionConfig, apiSaveCollectionConfig } from "../../utils/apiUrls"
import { Box, Flex, Alert, Button, Typography, Loader, Textarea, TextInput, ToggleInput } from '@strapi/design-system'
...
const PageConfigureCollection = () => {
    ...
    const reqSaveCollectionConfig = () => {
        if (collectionConfig && collectionConfig.collectionName) {
            ...
            .then((resp) => {
                toggleNotification({
                    type: "success", message: "The collection configuration is saved.",
                    timeout: 5000
                })
            })
            .catch((err) => {
                toggleNotification({
                    type: "warning", message: err.message || "An error was encountered.",
                    timeout: 5000
                })
                console.log(err)
            })
        ...
        if (selectedCollection) {
            ...
            .then((resp) => {
                if (Object.keys(resp).length === 0) {
                    toggleNotification({
                        type: "warning", message: 'No collection with the selected name exists.',
                        timeout: 5000
                    })
                } else {
                                    ...
                                    <Box padding={8} background="neutral0" width="100%">
                                        <Box paddingBottom={2}>
                                            ...
                                            { collectionConfig.attributes.map((a, idx) => {
                                                    return <Box paddingTop={4} paddingBottom={4}><ConfigureField index={idx} config={a} setFieldConfig={updateCollectionsConfig} /></Box>
                                                })
                                            }
                                        </Box>
                                    </Box>
